...
table(fyc2018$ADCOLN42, fyc2018$ADCLNS42)
```
...
## 10. Missing values Fixes

### remove NAs

check what colum have values -9, 

```{r}
-9  -7 -8  -15 

data %>%
  sapply(function(col) any(col == -9)) %>%
  which() %>%
  names()

# same as the above function
names(which( sapply(CDATA,function(col) any(col == -1))))


table(CDATA$EMPST31)
```

now removing these values


```{r}
any(is.na(data)) # check if there are any NA values in data

data<-CDATA
data[data == -9 | data == -7| data == -8 | data == -15] <- NA

data <- na.omit(data)

```

### check variable with -1 

check which variable have values == -1

```{r}
# Identify and display columns with the value -1
data %>%
  sapply(function(col) any(col == -1)) %>%
  which() %>%
  names()

```

 following variables have values -1 and thus need to be delt with : 
